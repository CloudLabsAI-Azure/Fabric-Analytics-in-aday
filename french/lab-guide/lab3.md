# Microsoft Fabric - Analyste Fabric en un jour - Laboratoire 3
...
5. La boîte de dialogue **Nouveau raccourci** s’ouvre alors. Sous **Sources externes**, sélectionnez **Azure Data Lake Storage Gen2**.
...
13. Vous êtes alors redirigé vers la boîte de dialogue suivante dans laquelle nous pouvons modifier les noms. Cliquez sur l’**icône Modifier** sous Actions pour **Application.Cities**.
...
3. Nous devons faire glisser les tables vers le volet Requête visuelle pour créer une requête. Faisons glisser les requêtes Cities, States et Countries dans le volet Requête visuelle.
   ...
   Nous devons fusionner ces requêtes. Et la requête visuelle est fournie avec la possibilité d’utiliser l’éditeur Power Query. Utilisons-le, puisqu’il nous est familier.
...
15. La boîte de dialogue Fusionner des requêtes s’ouvre alors. Dans le champ **Table de droite pour la fusion,** sélectionnez **Countries**.
...
25. **Cliquez avec le bouton droit sur la requête Merge** dans le volet (gauche) Requêtes. Sélectionnez **Renommer** et redéfinissez le nom de la requête sur **Geo**.

26. **Cliquez avec le bouton droit sur la requête Geo** dans le volet (gauche) Requêtes. Sélectionnez **Activer le chargement** pour activer cette requête.
...
3. **Sélectionnez la requête Customers**. Une fois sélectionnée, la requête Customers a une bordure bleue et un signe « + » s’affiche après Table. (Ce signe indique que nous ajoutons une étape après Table. Si vous ne voyez pas le signe « + » après Table, vous avez peut-être sélectionné une autre étape. Sélectionnez Table et vous serez prêt.)
...
6. Sélectionnez la colonne **BuyingGroupID** dans les deux tables. Nous allons les joindre à l’aide de cette colonne.
...
8. Sélectionnez la colonne **InvoiceID** dans les deux tables. Nous allons les joindre à l’aide de cette colonne.
...
11. Si vous ne l’avez pas encore ouvert, ouvrez le fichier **FAIAD.pbix** situé dans le dossier **Reports** sur le bureau de votre environnement de labo. 
...
29. Cliquez sur **Enregistrer** pour enregistrer et fermer la boîte de dialogue Power Query. Vous êtes alors redirigé vers la requête visuelle.
...
3. Dans l’éditeur de requête visuelle, cliquez sur l’**icône Mode Focus** pour ouvrir l’éditeur Power Query.
...
7. Sélectionnez la colonne **StockItemID** dans les deux tables. Nous allons les joindre à l’aide de cette colonne.
...
16. Cliquez avec le bouton droit sur la requête Product et sélectionnez **Activer le chargement** pour permettre le chargement de la requête.
...
18. Dans le menu Requête visuelle, sélectionnez E**nregistrer en tant que vue**. La boîte de dialogue Enregistrer en tant que vue s’ouvre alors. Notez que la requête SQL est disponible. Vous pouvez l’examiner, le cas échéant. 
