...
1. From the top menu, select **New -> Dataflow Gen2**.
      ...
      ![A screenshot to select New -> Dataflow Gen2](../media/Picture91.png)
...
1. **Right click** and select **Copy**.
     ...
     ![A screenshot to copy queries from Power Query window](../media/Picture92.png)
...
1. Select **Configure connection**.
     ...
     ![A screenshot of dataflow to configure connection](../media/Picture93.png)
...
1. **Username** should be **TE_SNOWFLAKE**.
1. **Password** should be **)EHZ3h7!eGym8EWR**.
1. Select **Connect**.

     ![A screenshot to connect to data source](../media/Picture94.png)
     ...
     SupplierCategoreis as the name implies has supplier categories. These two tables are joined to create Supplier dimension, with the columns we need. Similarly, we have PO Line Items merged with PO to create the 
    PO fact. Now we need to ingest Supplier and PO data into Lakehouse.
...
1. As mentioned earlier, we do are not Staging any of this data. So right click on **Supplier** query in the Queries pane and select **Enable staging** to remove the check mark.
      ...
      ![A screenshot to Enable Staging](../media/Picture95.png)
...
1. Select Lakehouse from the dialog.
      ...
      ![A screenshot to select Data Destination for Supplier query](../media/Picture96.png)

1. Connect to data destination dialog opens. From the **Connection dropdown** select **Lakehouse (none)**.

      ![A screenshot to Connect to data destination](../media/Picture97.png)
...
1. Select **Next**.
      ...
      ![A screenshot to Choose destination target](../media/Picture98.png)
...
1. Notice there is a warning. Lakehouse does not support column names with space in it. Select **Fix it**, to fix the warning.
      ...
      ![A screenshot to Choose destination settings](../media/Picture99.png)
...
1. Similarly, set up the Data Destination for PO query. Once it is done, your PO query should have Data Destination set to Lakehouse as show in the screenshot below.
     ...
     ![A screenshot showing Data Destintion for PO](../media/Picture100.png)
...
1. Click on **Enter** to save the name change.
      ...
      ![A screenshot showing renaming of Dataflow1](../media/Picture101.png)
   
1. On the bottom right corner, select **Publish**.
   
      ![A screenshot to Publish Dataflow](../media/Picture102.png)
...
1. Once Dataflow 1 is finished publishing, let’s rename it. Click on the **ellipsis** next to Dataflow 1. Select **Properties**.
      ...
      ![A screenshot to select Properties for Dataflow1](../media/Picture103.png)
...
1. Select **Save**.
      ...
      ![A screenshot of Properties dialog of Dataflow1](../media/Picture104.png)
...
1. From the top menu, select **New -> Dataflow Gen2**.
      ...
      ![A screenshot to select New -> Dataflow Gen2](../media/Picture105.png)
...
1. **Right click** and select **Copy**.
      ...
      ![A screenshot copy queries from Power Query window](../media/Picture106.png)
...
1. Select **Configure connection**.
     ...
     ![A screenshot Configure connection in Dataflow](../media/Picture107.png)
...
1. Enter the below credentials to connect to the Dataverse:
    ...
    ![A screenshot of Connect to data source](../media/Picture108.png)
...
1. As mentioned earlier, we do are not Staging any of this data. So right click on **Customer** query in the Queries pane and select **Enable staging** to remove the check mark.
     ...
     ![A screenshot to disable Staging](../media/Picture109.png)

1. Select **Customer** query.
1. On the bottom right corner select “**+**” next to **Data destination**.
1. Select Lakehouse from the dialog.
   
      ![A screenshot to configure Data Destination for Customer query](../media/Picture110.png)

1. Connect to data destination dialog opens. From the **Connection dropdown** select **Lakehouse (none)**.

      ![A screenshot of Connect to data destination](../media/Picture111.png)
...
1. Select **Next**.
      ...
      ![A screenshot of Choose destination target](../media/Picture112.png)

1. Choose destination settings dialog opens. Every time Dataflow Gen2 is refreshed we would like to perform a full load. Make sure Update method is set to **Replace**.
1. Notice there is a warning. Lakehouse does not support column names with space in it. Select **Fix it**, to fix the warning.
   
     ![A screenshot of Choose destination settings](../media/Picture113.png)

1. Column mapping can be used to map dataflow columns to existing columns. In our case, it is a New Table. Hence, we can use the defaults. Select **Save settings**.
1. You will be navigated back to Power Query window. Notice on the bottom **right corner**, Data destination is set to Lakehouse.
1. On the bottom right corner, select **Publish**.
   
      ![A screenshot to Publish queries](../media/Picture114.png)
...
1. Dataflow 1 is the dataflow we were working in. Let’s rename it before we continue. Click on the **ellipsis** next to Dataflow 1. Select **Properties**.
      ...
      ![A screenshot to select Dataflow1 -> Properties](../media/Picture115.png)
...
1. Select **Save**.
      ...
      ![A screenshot of Properties dialog of Dataflow1](../media/Picture116.png)
...
1. From the top menu, select **New -> Dataflow Gen2**.
      ...
      ![A screenshot to select New -> Dataflow Gen2](../media/Picture117.png)
...
1. **Right click** and select **Copy**.
      ...
      ![A screenshot to copy queries from Power Query window](../media/Picture118.png)
...
1. Select **Configure connection**.
      ...
      ![A screenshot to configure connection for People query in Dataflow](../media/Picture119.png)

1. Connect to data source dialog opens. In the **Connection** dropdown make sure, **Create new connection** is selected.
1. **Authentication kind** should be **Organizational Account**.
1. Select **Connect**.

      ![A screenshot of Connect to data source](../media/Picture120.png)
...
1. As mentioned earlier, we do are not Staging any of this data. So right click on **People** query in the Queries pane and select **Enable staging** to remove the check mark.
     ...
     ![A screenshot to disable Staging](../media/Picture121.png)

1. Select **People** query.
1. On the bottom right corner select “**+**” next to **Data destination**.
1. Select Lakehouse from the dialog.
   
      ![A screenshot to configure Data Destination for People query](../media/Picture122.png)

1. Connect to data destination dialog opens. From the **Connection dropdown** select **Lakehouse (none)**.

      ![A screenshot of Connect to data destination](../media/Picture123.png)
...
1. Select **Next**.
    ...
    ![A screenshot of Choose destination target](../media/Picture124.png)

1. Choose destination settings dialog opens. Every time Dataflow Gen2 is refreshed we would like to perform a full load. Make sure Update method is set to **Replace**.
1. Notice there is a warning. Lakehouse does not support column names with space in it. Select **Fix it**, to fix the warning.
   
     ![A screenshot of Choose destination settings](../media/Picture125.png)

1. Column mapping can be used to map dataflow columns to existing columns. In our case, it is a New Table. Hence, we can use the defaults. Select **Save settings**.
1. You will be navigated back to Power Query window. Notice on the bottom **right corner**, Data destination is set to Lakehouse.
1. On the bottom right corner, select **Publish**.
   
     ![A screenshot of publishing People query](../media/Picture126.png)

     You will be navigated back to Data Factory screen. It may take a few moments for the Dataflow to Publish. 

1. Dataflow 1 is the dataflow we were working in. Let’s rename it before we continue. Click on the **ellipsis** next to Dataflow 1. Select **Properties**.

      ![A screenshot of Dataflow1 -> Properties](../media/Picture127.png)
...
1. Select **Save**.
      ...
      ![A screenshot of Dataflow1 dialog](../media/Picture128.png)
...
Fabric Analyst in a Day introduces you to some of the key functions available in Microsoft Fabric. In the menu of the service, the Help section has links to some great resources.
   ...
   ![A screenshot of help options](../media/Picture129.png)
