...
서문	3
데이터 흐름 2 세대	3<br>
작업 1: 데이터 흐름 2 세대 만들기	3<br>
작업 2: ADLS Gen2 에 대한 연결 만들기	5<br>
작업 3: 기본 ADLS Gen2 폴더 쿼리 만들기	8<br>
작업 4: Cities 쿼리 만들기	9<br>
작업 5: Countries 쿼리 만들기	12<br>
작업 6: 복사를 사용하여 States 생성 - 옵션 1	14<br>
작업 7: 복사로 Geo 쿼리 만들기 - 옵션 2	16<br>
작업 8: Geo 쿼리를 위한 데이터 대상 구성	19<br>
작업 9: 데이터 흐름 게시	22<br>
작업 10: 데이터 흐름 이름 바꾸기	24<br>
작업 11: 데이터 흐름에서 나머지 쿼리 빌드	25<br>
작업 12: 나머지 쿼리를 위한 데이터 대상 구성	28<br>
참조	31<br>
...
**ADLS Base Folder** 쿼리를 마우스 우클릭합니다.<br>
...
4.	**필터 행** 대화 상자에 **Application.Cities**를 입력합니다.<br>

**참고:** 이 대화 상자는 대소문자를 구분합니다.<br>
...
7.	모든 도시 상세 정보가 표시됩니다. **오른쪽 패널의 쿼리 설정 -> 속성 -> 이름**에서 이름을
**Cities**로 변경합니다.

     ![](../Images/Lab-03/image054.jpg)<br>
...
4.	**필터 열 대화 상자에 Application.Countries**를 입력합니다.<br>
...
11.	쿼리 삭제 대화 상자가 나타납니다. **삭제**를 선택하여 삭제합니다.<br>
...
옵션을 확인해 보겠습니다.<br>
기존 테이블에 **데이터를 추가**하거나 **교체**하는 옵션이 있습니다.<br>
...
지원하지 않습니다. 수정을 선택하면 열 이름의 공백을 대체하기 위해 밑줄이 추가됩니다.<br>
...
작업을 쉽게 하기 위해서 Power BI Desktop에서 쿼리를 복사할 수 있는지 살펴보겠습니다.<br>
2.	아직 열지 않았다면 랩 환경의 **C:\FAIAD\Reports** 폴더에 있는 **FAIAD.pbix**를 엽니다.
3.	리본 메뉴에서 **홈 -> 데이터 변환**을 선택합니다. Power Query 창이 열립니다.
 
4.	왼쪽의 **쿼리** 패널에서 **ADLSData**로부터 다음 쿼리를 **Ctrl+선택**합니다.<br>
a.	Product<br>
b.	Product Groups<br>
...
8.	앞서 언급했듯이 우리는 이 데이터를 스테이징하지 않습니다. 따라서 다음 쿼리를 **마우스 우클릭**하고 **스테이징 사용**을 선택하여 확인 표시를 제거합니다.<br>
a.	Product<br>
b.	제품 상세 정보<br>
c.	Reseller<br>
d.	영업<br>
...
12.	마찬가지로 다음 쿼리에 대해 **데이터 대상**을 설정합니다.<br>
